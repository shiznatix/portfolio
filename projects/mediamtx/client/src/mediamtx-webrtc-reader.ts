/* eslint-disable */
// @ts-nocheck
/** WebRTC/WHEP reader configuration */
...
class MediaMTXWebRTCReader {
	...
	private state: 'get-codecs' | 'closed' | 'running' | 'restarting' | 'failed';
	...
	static #supportsNonAdvertisedCodec(codec: any, fmtp: any): Promise<boolean> {
	...
	static #linkToIceServers(links: string | null): any[] {
		return (links !== null) ? links.split(', ').map((link: string) => {
	...
	getStats(): Promise<RTCInboundRtpStreamStats | void> {
		if (this.state !== 'running') {
			return;
		}
